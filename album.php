...
<main>
    ...
    <div class="AlbumList">
    <!-- <form class="form-input-row" action="./album.php">
        <input type="text" name="cari" placeholder="Cari Album..." class="inptxt" tabindex="1" required>
        <button type="submit" name="onsearch" class="SearchButton">Cari</button>
    </form> -->
    <?php
    if (isset($_GET['onsearch'])) {
        $cari = $_GET['cari'];
        $query = $connect->query("SELECT * FROM album WHERE NamaAlbum LIKE '".$cari."' ORDER BY TanggalDibuat DESC;");
        if($query->num_rows > 0){
